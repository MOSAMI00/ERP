...
const fallbackTenant = (rental) =>
    rental?.tenant ?? { name: "مستخدم غير معروف", rating: 0 };

const isPaidRental = (rental) =>
...
const RecentOrdersTable = ({ rentals, isLoading }) => {
    const handleApprove = (rental) => {
        if (
            confirm(
                "هل أنت متأكد من الموافقة على هذا الطلب؟ سيتم توقيع العقد آلياً.",
            )
        ) {
    ...
    const columns = [
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-[#F39C12]">
                                ⭐ {Number(tenant.rating ?? 0).toFixed(1)}
                            </div>
                        </div>
                    </div>
                );
        ...
        },
        {
            ...
            header: "المعدة",
            cell: (rental) =>
                fallbackEquipment(rental).name ?? "معدة غير معروفة",
        },
        {
            key: "period",
            header: "الفترة",
            cell: (rental) =>
                `${rental.start_date || rental.startDate || "—"} - ${rental.end_date || rental.endDate || "—"}`,
        },
        {
            key: "total",
            header: "المبلغ",
            cell: (rental) =>
                `${formatCurrency(rental.total_amount || rental.totalAmount || 0)} ر.ي`,
        },
        {
            key: "status",
            header: "الحالة",
            ...
            cell: (rental) =>
                rental.status === "pending" ? (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {["confirmed"].includes(rental.status) &&
                        !isPaidRental(rental) ? (
                            <AppButton
                                variant="danger"
                                size="icon"
    ...
    return (
        <div className="owner-card bg-white p-6 rounded-xl border border-border">
            <div className="flex items-center justify-between mb-6">
                ...
                <button
                    onClick={() => router.visit("/owner/requests")}
